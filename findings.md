...
- In multilingual clinical consultations (Urdu/English), Whisper STT can default to Devanagari (Hindi) script without explicit language locking (`language: "ur"` and Urdu prompt). A dedicated transliteration layer (`devanagariToUrdu`) ensures all transcript provenance renders strictly in Urdu (Perso-Arabic) or English.
- Note schema polymorphic states (`NoteDraft` vs `ApprovedNote`) require careful type handling in LLM transformers so approved note fields (like `id`, `approved_at`, and `approval_status`) are preserved across AI rewrites and translations.