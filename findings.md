...
- The partially added manual-editor conditional requires its outer close, `SessionWorkspaceViewProps` must declare `isManualEntry`, and its Transcript tab must use `session_info.transcript_source` rather than claiming every session came from Whisper audio.
