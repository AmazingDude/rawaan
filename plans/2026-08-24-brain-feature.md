...
**Tech Stack:** Next.js 16 App Router + TypeScript strict (existing), Zod 4 (existing), Vitest (existing), Playwright (existing). New: zero npm dependencies. LLM access via plain `fetch` against the Anthropic Messages API or OpenAI Chat Completions API (documented REST endpoints), selected by env vars — no SDK, no vector database, no new storage engine.
...
| Decision | Choice | Why |
|---|---|---|
| LLM access | Plain `fetch`, two adapters (`anthropic`, `openai`) behind one interface | Zero new dependencies; endpoints are stable documented REST APIs; easy to fake in tests via a one-method interface |
| Provider selection | `LLM_PROVIDER=anthropic\|openai` + `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` in `.env.local`; `LLM_MODEL` overrides default | PRD §8.1 allows GPT-4-class or Claude; team can switch without code changes |
...
- Secrets only in `.env.local` (gitignored); never logged. `.env.example` gains placeholder keys.
...
| `lib/llm/provider.ts` | `LlmCompletionProvider` interface + anthropic/openai fetch adapters + env factory | Aashir |
...
### Task 4: LLM provider boundary with env-selected adapter (Aashir)
...
**Interfaces:**
- Produces: `interface LlmCompletionProvider { complete(input: { system: string; user: string }): Promise<string> }`; `createLlmProviderFromEnv(env: NodeJS.ProcessEnv): LlmCompletionProvider` (throws descriptive `Error` when config missing); `createAnthropicProvider(apiKey: string, model: string)`; `createOpenAiProvider(apiKey: string, model: string)`

- [ ] **Step 1: Write failing tests** — use `vi.stubGlobal("fetch", …)` doubles; no real network in unit tests:

```ts
it("throws a clear error when no provider or key is configured", () => {
  expect(() => createLlmProviderFromEnv({})).toThrowError(/LLM provider is not configured/);
});

it("anthropic adapter sends the documented request shape and returns text", async () => {
  const fetchMock = vi.fn().mockResolvedValue(
    new Response(JSON.stringify({ content: [{ type: "text", text: "hello" }] }), { status: 200 }),
  );
  vi.stubGlobal("fetch", fetchMock);
  const provider = createAnthropicProvider("key", "claude-default-model");
...
it("aborts after the 30s bound instead of hanging", async () => {
  ...
  vi.stubGlobal("fetch", fetchMock);
  const provider = createOpenAiProvider("key", "gpt-default-model");
  await expect(provider.complete({ system: "s", user: "u" })).rejects.toThrowError();
});
```

- [ ] **Step 2: Verify failure**
- [ ] **Step 3: Implement** — both adapters: build request, `AbortSignal.timeout(30_000)`, one retry on network-level failure (not on HTTP 4xx), non-2xx → throw `Error(\`LLM provider returned ${status}\`)`, extract text (`content[].type==="text"` join for Anthropic; `choices[0].message.content` for OpenAI). Factory:

```ts
export function createLlmProviderFromEnv(env: Pick<NodeJS.ProcessEnv, "LLM_PROVIDER" | "ANTHROPIC_API_KEY" | "OPENAI_API_KEY" | "LLM_MODEL">): LlmCompletionProvider {
  if (env.LLM_PROVIDER === "anthropic") {
    if (!env.ANTHROPIC_API_KEY) throw new Error("LLM provider is not configured: ANTHROPIC_API_KEY missing");
    return createAnthropicProvider(env.ANTHROPIC_API_KEY, env.LLM_MODEL ?? "claude-sonnet-4-5");
  }
  if (env.LLM_PROVIDER === "openai") {
    if (!env.OPENAI_API_KEY) throw new Error("LLM provider is not configured: OPENAI_API_KEY missing");
    return createOpenAiProvider(env.OPENAI_API_KEY, env.LLM_MODEL ?? "gpt-4.1-mini");
  }
  throw new Error("LLM provider is not configured: set LLM_PROVIDER to 'anthropic' or 'openai' in .env.local");
}
```

> **API check before implementing (AGENTS.md rule 1):** confirm current endpoint/request fields against docs.anthropic.com/api/messages and platform.openai.com/docs/api-reference/chat before merging — model IDs are env-overridable so defaults can lag safely.

Create `.env.example` with commented placeholders (no real values).
